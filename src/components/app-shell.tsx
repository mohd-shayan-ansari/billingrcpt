...
function receiptStatusLabel(receipt: ReceiptRecord) {
  return receipt.syncStatus === "pending" ? "Syncing" : "Synced";
}
...
export function AppShell() {
  ...
  async function prefetchNextReceiptNumber() {
    try {
      ...
      // ignore errors, keep existing next receipt number
    }
  }
  ...
  async function resetAllReceipts() {
    if (session?.role !== "MASTER_ADMIN") {
      setMessage("Only master admin can reset all receipts");
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        setMessage(data.error ?? "Unable to reset receipts");
        return;
      }

      window.localStorage.removeItem(PENDING_RECEIPTS_STORAGE_KEY);
      pendingReceiptSyncInFlight.current = false;
      setReceipts([]);
      setLastReceipt(null);
      ...
      {currentPage === "settings" && (
        <div className="mb-6 flex flex-col gap-4 rounded-[2rem] border border-white/10 bg-white/5 p-5 backdrop-blur md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-400">Manage the app, receipt storage, and account settings.</p>
          </div>
          <div className="flex flex-wrap items-center gap-3">
            {session.role === "MASTER_ADMIN" && (
              <button onClick={() => void resetAllReceipts()} disabled={resetReceiptsLoading} className="rounded-2xl border border-red-300/20 bg-red-400/10 px-4 py-3 font-medium text-red-100 transition hover:bg-red-400/20 disabled:opacity-60">
                {resetReceiptsLoading ? "Resetting..." : "Reset All Receipts"}
              </button>
            )}
